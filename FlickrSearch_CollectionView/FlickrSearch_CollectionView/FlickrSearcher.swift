...
let apiKey = "b28938be182878184f52bf8d6facbe2e"
...
class FlickrPhoto : Equatable {
  ...
  func flickrImageURL(size size:String = "m") -> NSURL {
...
class Flickr {
  ...
  func searchFlickrForTerm(searchTerm: String, completion : (results: FlickrSearchResults?, error : NSError?) -> Void){
    ...
    NSURLConnection.sendAsynchronousRequest(searchRequest, queue: processingQueue) {response, data, error in
      ...
      do {
        resultsDictionary = try NSJSONSerialization.JSONObjectWithData(data!, options: .AllowFragments) as! NSDictionary
