...
func highlightWords(pattern: String, inString string: String) -> NSAttributedString {
  let regex = try? NSRegularExpression(pattern: pattern, options: [])
  let range = NSMakeRange(0, string.characters.count)
  let matches = (regex?.matchesInString(string, options: [], range: range))! as [NSTextCheckingResult]
  
  let defaultAttributes = [NSFontAttributeName: UIFont.boldSystemFontOfSize(14.0)]
  let attributedText = NSMutableAttributedString(string: string, attributes: defaultAttributes)
  
  for match in matches {
    attributedText.addAttribute(NSBackgroundColorAttributeName, value: UIColor.yellowColor(), range: match.range)
    attributedText.addAttributes([NSFontAttributeName: UIFont.systemFontOfSize(12.0)], range: match.range)
  }
...
let testStrings = "Michael R created a new post. Paul S, Nina B and 3 others also favorited your post. You, Rex and 4 others also commented on Paul F's post. John D and Mikey M commented on your post. Nina B would like to say Hi. Paul S, Nina B and 3 others also favorited Paul S's post. Joed: Hey! Are you there? Yes."

let pPost = "created a new post"
let pComments = "(other(s)? also)? commented on your post|(other(s)? also)? commented on \\w+ \\w+('s)? post"
let pFavorited = "(other(s)? also)? favorited your post|(other(s)? also)? favorited \\w+ \\w+('s)? post"
let pFollow = "followed you|(other(s)? also)? followed you"
let pHi = "would like to say Hi"
let pMessage = ":.{0,}"
let pConjs = "\\.|\\,|\\sand\\s"

let patts = "\(pPost)|\(pComments)|\(pFollow)|\(pFavorited)|\(pHi)|\(pConjs)|\(pMessage)"

highlightMatches(patts, inString: testStrings)

highlightWords(patts, inString: "John D would like to say Hi")

highlightWords("created a new post|others also favorited your post|\\sand\\s|commented on \\w+ \\w+'s post|also commented on \\w+ \\w+'s post|would like to say Hi|:.{0,}|\\.|\\,", inString: testStrings)


let chatString = "Joed: Hey! Are you there? Yes"
highlightMatches(":.{0,}", inString: chatString)
